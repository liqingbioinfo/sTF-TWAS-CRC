...
wk="/data/sbcs/GuoLab/backup/liq17/CRC_TF_TWAS_zhishan2024/NC_revision_OceanCode/NC_revision"
#beta1=args[1]
beta1=1
h2=args[1]
gwas_snp="10M"
...
###load sim_ys
y_sim=data.frame(fread(paste0(wk, "/simulation_values_res/sim_y_values/beta1_",beta1,"_h2_",h2,".sim_y.txt"), header = TRUE))
tmp=colnames(y_sim)[2:length(colnames(y_sim))]
tracks_num = sub("^X", "", tmp)

###load annots
annots=data.frame(fread(paste0(wk,"/84Tracks/multi_annots_ldscores/annot_",gwas_snp,"/1_22.annot.txt")))
...
for(y_track in tracks_num){
	y_track_name=paste0("X", y_track)
	gwas_file=paste0(wk,"/simulation_values_res/sim_y_values/beta1_",beta1,"_h2_",h2,".sim_y/",gwas_snp,".",y_track,".glm.logistic.hybrid")
	###load GWAS SS
	if(file.exists(gwas_file)){
		...
		fwrite(ldsc_gwas_ss, paste0(wk, "/simulation_values_res/sim_y_values/beta1_",beta1,"_h2_",h2,".sim_y/",gwas_snp,".",y_track,".glm.logistic.ldsc.txt"), sep="\t")
...
fwrite(as.data.frame(glm_res), paste0(wk, "/simulation_values_res/lmer_res/beta1_",beta1,"_h2_",h2,".sim_y.lmer.csv"))
