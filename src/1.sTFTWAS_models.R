...
###### II. input files ######
# snp annotation
snp_annot_file <- paste0("../data/For_sTFTWAS_model/chr",chrom,".TF_variants_annotation.txt")
# genotype file
genotype_file <- paste0("../data/For_sTFTWAS_model/chr",chrom,"_example.gt.gt") #Through dbGap
# vcf head
vcfhead <- read.table(paste0("../data/For_sTFTWAS_model/chr",chrom,"_example.header.txt"),header = F, stringsAsFactors = F)
vcfhead <- as.character(vcfhead[1,])
# gene annotation
gene_annot_file <- "../data/For_sTFTWAS_model/v26.gtf.annot.txt" #download from gencode URL: https://www.gencodegenes.org/human/
# gene expression
expression_file <- "../data/For_sTFTWAS_model/chr22.example.expression.tsv"
...
{
    gene_df <- read.table(gene_annot_file,header=T,stringsAsFactors =F,sep="\t",fill=T)
    gene_df1 <- filter(gene_df,gene_df$anno %in% "gene")
    geneid <- str_extract(gene_df1$geneid, "ENSG\\d+.\\d+")
	gene_used <- gene_df1[c("geneid","genename","chr","start","end","anno")]
